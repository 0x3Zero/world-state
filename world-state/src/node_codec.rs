...
impl Decode for NodeHeader {
    ...
    let byte_decode = input.read_byte()?;
    // println!("byte_decode: {:?}", byte_decode);
		Ok(match byte_decode {
			EMPTY_TRIE => {
				NodeHeader::Null
			},
...
fn partial_from_iterator_to_key<I: Iterator<Item = u8>>(
  ...
  // println!("nibble_count: {:?}, offset: {:?}, over: {:?}", nibble_count, offset, over);
